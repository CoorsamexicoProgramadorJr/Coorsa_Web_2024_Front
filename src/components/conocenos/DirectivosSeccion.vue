...
<template>
  <section class="w-screen h-screen  flex flex-col lg:flex-row pt-[8vh] md:pt-[5%] overflow-hidden ">
    <div class="lg:w-1/3 w-full lg:h-full h-1/3 px-[3%]">
      ...
      <p class="2xl:text-5xl xl:text-4xl lg:text-3xl md:text-2xl text-xl lg:mt-[15%] mt-4 font-light text-center md:text-left">
        Conoce al <span class="font-medium">increíble equipo</span> detrás de este proyecto y descubre más sobre cómo trabajamos.
      </p>
    </div>
    <!-- Directives Carousel -->
    <div class="relative w-full lg:w-2/3 lg:h-full h-2/3">
      <div v-show="show" class="f-carousel lg:w-full w-[90%] lg:h-[80%] h-[90%] text-white mx-auto" id="directivosCarousel">
        <template v-for="directive in directivesStore.directives" :key="directive.id">
          <div v-if="directive.active" class="flex flex-col justify-end overflow-hidden bg-top bg-cover rounded-xl f-carousel__slide" :data-lazy-src="directive.image">
