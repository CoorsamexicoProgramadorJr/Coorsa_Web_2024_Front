...
<template>
  <div class="absolute top-0 left-0 w-screen h-screen bg-black/50 lg:w-3/4 z-[1] lg:ml-[25%]">
    <div class="w-[85%] mx-auto top-[5%] lg:top-[7%] max-h-[92vh] bg-white z-[3] relative rounded-2xl p-4 overflow-y-auto md:w-[70%] lg:w-2/3">
      <button @click="appAdmStore.manageApplicationDetails()" class="fixed right-[7%] md:right-[15%] lg:right-[13%] top-[5%] lg:top-[7%] flex items-center justify-center text-blue-900 z-[4]">
      ...
      <h3 class="text-3xl font-bold text-blue-900 border-b border-blue-900 lg:text-4xl">Detalles de Postulación</h3>
      <p class="float-right mt-2 text-base text-blue-800"> {{ formatDate(appAdmStore.application.created_at) }}</p>
      ...
      <div class="py-2 space-y-2">
        <div>
          ...
          <p class="ml-4 text-base">{{ appAdmStore.application.name }}</p>
        </div>
        <div>
          <div class="flex items-center gap-2 text-blue-900">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
          ...
          <p class="ml-4 text-base">{{ appAdmStore.application.email }}</p>
        </div>
        <div>
          <div class="flex items-center gap-2 text-blue-900">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
          ...
          <p class="ml-4 text-base">+{{ contactStore.getAreaCode(appAdmStore.application.area_code_id) }} {{ appAdmStore.application.phone.replaceAll('-', ' ') }}</p>
        </div>
        <div>
          <div class="flex items-center gap-2 text-blue-900">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
