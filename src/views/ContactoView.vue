...
<template>
  <section class="flex w-screen h-screen text-white bg-black">
    ...
    <article class="lg:w-2/3 h-full pt-[8vh] md:px-[5%] px-[10%]">
      <!-- Contact form -->
      <form action="" v-if="vacancyId.length == 0" class="h-[80%] md:pt-[5%] pt-3 grid grid-cols-2 md:grid-rows-6 grid-rows-8 lg:gap-x-[3%] md:gap-x-[5%] 2xl:text-2xl xl:text-xl text-lg">
        ...
        <div class="flex flex-col justify-center w-full md:gap-[8%] md:col-span-1 col-span-2">
          ...
          <fieldset class="flex w-full h-[40%] text-black">
            <select name="lada" id="lada" class="2xl:w-[38%] xl:w-[40%] lg:w-[45%] md:w-[39%] w-[28%] border-l border-y focus:border-l-2 focus:border-y-2 border-red-700 md:rounded-l-xl rounded-l-full px-[2%] outline-none 2xl:text-xl xl:text-lg text-base md:tracking-tighter tracking-tight">
              <option v-for="state in states" :key="state.name" :value="state.lada" :title="state.name">{{ state.code }} ({{ state.lada }})</option>
            </select>
            <input type="tel" id="phone" name="phone" placeholder="+ 1 (555) 000 0000" pattern="[0-9]{3}-[0-9]{2}-[0-9]{3}" class="2xl:w-[62%] xl:w-[60%] lg:w-[55%] md:w-[61%] w-[72%] border-r border-y focus:border-r-2 focus:border-y-2 border-red-700 outline-none md:rounded-r-xl rounded-r-full px-[3%]">
          </fieldset>
        </div>
        ...
        <div class="flex flex-col justify-center w-full gap-[8%] md:col-span-1 col-span-2">
          ...
          <select name="service" id="service" class="h-[40%] w-full md:rounded-xl rounded-full outline-none border focus:border-2 border-red-700 px-[1%] text-black text-center uppercase 2xl:text-xl xl:text-lg lg:text-sm xl:tracking-tight">
        ...
        <div class="flex flex-col justify-center w-full gap-[8%] md:col-span-1 col-span-2">
          ...
          <select name="state" id="state" class="h-[40%] w-full md:rounded-xl rounded-full outline-none border focus:border-2 border-red-700 px-[1%] text-black text-center uppercase 2xl:text-xl xl:text-lg lg:text-sm">
