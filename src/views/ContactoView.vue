...
<template>
  <section class="flex w-screen text-white bg-black md:h-screen h-[120vh]">
    ...
    <article class="lg:w-2/3 h-full pt-[8vh] md:px-[5%] px-[10%]">
      <!-- Contact form -->
      <ContactAlert v-if="getAlertStatus()" alert-type="consulta"/>
      <form action="" v-if="vacancyId.length == 0" @submit.prevent="submitContactForm" class="h-[85%] md:pt-[5%] pt-3 grid grid-cols-2 md:grid-rows-6 grid-rows-8 lg:gap-x-[3%] md:gap-x-[5%] gap-y-2 sm:gap-y-0 2xl:text-2xl xl:text-xl text-lg" novalidate>
        <div class="flex flex-col justify-center w-full col-span-2 md:col-span-1">
          ...
          <p v-if="Object.keys(errors).length != 0 && Object.keys(errors.errors).includes('name')" class="text-sm text-red-700 sm:text-base">{{ errors.errors.name[0] }}</p>
        </div>
        <div class="flex flex-col justify-center w-full col-span-2 md:col-span-1">
          ...
          <p v-if="Object.keys(errors).length != 0 && Object.keys(errors.errors).includes('area_code_id') || Object.keys(errors).length != 0 && Object.keys(errors.errors).includes('phone')" class="text-sm text-red-700 sm:text-base">
        ...
        <div class="flex flex-col justify-center w-full col-span-2">
          ...
          <p v-if="Object.keys(errors).length != 0 && Object.keys(errors.errors).includes('email')" class="text-sm text-red-700 sm:text-base">{{ errors.errors.email[0] }}</p>
        </div>
        <div class="flex flex-col justify-center w-full col-span-2 md:col-span-1">
          <label for="service" class="mb-1 font-semibold uppercase">Servicio</label>
          <select name="service" id="service" v-model="contactForm.service_id" class="max-h-[50%] w-full md:rounded-xl rounded-full outline-none border focus:border-2 border-red-700 px-[1%] text-black text-center uppercase 2xl:text-xl xl:text-lg lg:text-base xl:tracking-tight py-1">
            <option value="" selected>-- Selecciona una opción --</option>
            <option v-for="service in services" :key="service.id" :value="service.id">{{ service.name }}</option>
          </select>
          <p v-if="Object.keys(errors).length != 0 && Object.keys(errors.errors).includes('service_id')" class="text-sm text-red-700 sm:text-base">{{ errors.errors.service_id[0] }}</p>
        </div>
        <div class="flex flex-col justify-center w-full col-span-2 md:col-span-1">
          <label for="state" class="mb-1 font-semibold uppercase">Estado</label>
          <select name="state" id="state" v-model="contactForm.state_id" class="max-h-[50%] w-full md:rounded-xl rounded-full outline-none border focus:border-2 border-red-700 px-[1%] text-black text-center uppercase 2xl:text-xl xl:text-lg lg:text-base py-1">
            <option value="">-- Selecciona una opción --</option>
            <option v-for="state in states" :key="state.id" :value="state.id">{{ state.name }}</option>
          </select>
          <p v-if="Object.keys(errors).length != 0 && Object.keys(errors.errors).includes('state_id')" class="text-sm text-red-700 sm:text-base">{{ errors.errors.state_id[0] }}</p>
        </div>
        <div class="flex flex-col justify-center w-full col-span-2 row-span-2">
          ...
          <p v-if="Object.keys(errors).length != 0 && Object.keys(errors.errors).includes('message')" class="text-sm text-red-700 sm:text-base">{{ errors.errors.message[0] }}</p>
        </div>
